...
uniform vec3 iMouse; 
uniform vec4 iDate; 
//https://www.shadertoy.com/view/lss3zn
...
void main(void)
{
	...
	//use this to stay fullscreen on a single effect
	//float scale = 1.;
...
//	position = rotated_position;
	...
	for (float ii=0.0; ii<=recursion_level; ii++)
	{
		...
		//use this to visualize specific effects (use mouse to scroll to it)
		//if (effect_number != 15.0)break;
		...
		//deformed planes (top&bottom) with sin grid texture
		...
		//tunnel deform with sin grid testure
		...
		//Rotating+zooming checkerboard with rgb raster lines
		...
		//ball deform (curvilinear) with simple sin texture
